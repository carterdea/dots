...
3. Download each accessible attachment with:
   ...
   ```bash
   trello attachments download --card <card-id> --attachment <attachment-id> --output /tmp/<workflow-name>-<card-short-id>/
   ```

4. Use `--force` only when re-running and intentionally replacing a prior download.
5. Inspect downloaded attachments before deciding what to build, QA, or report:
...
6. Record attachment findings and use them alongside the card description, comments, checklists, labels, and acceptance criteria. If an attachment cannot be downloaded or opened, record the reason and continue only if the remaining card context is sufficient.
