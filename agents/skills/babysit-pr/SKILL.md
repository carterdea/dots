...
5. Finish only when both passes describe the same current head, CI is green, GitHub reports `mergeable: MERGEABLE`, no review is pending, no actionable feedback remains, and review is approved or the quiet period below has elapsed. Before reporting completion, fetch review state again with the review specialist's helper and refresh remote head, checks, and mergeability. New or edited feedback, pending reviews, or changed approval invalidate the candidate result even when the head is unchanged; return to the review pass and reset the quiet period. Keep polling while mergeability is unknown; route conflicts through the review specialist and stop with a blocker if they cannot be safely resolved.
...
The initial observation starts the clock with the clean-poll count at zero. The quiet period requires four subsequent clean polls, each after a five-minute wait, spanning at least 20 minutes since the last reset. A clean poll requires a mergeable PR, green CI, no pending review, and no unresolved actionable feedback. Unknown mergeability or pending or unknown CI prevents completion and breaks consecutive clean polls.
