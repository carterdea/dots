...
5. Finish only when both passes describe the same current head, CI is green, no review is pending, no actionable feedback remains, and review is approved or the quiet period below has elapsed. Verify the remote head is still that SHA before reporting completion.
...
The quiet period requires four clean polls separated by five minutes, spanning at least 20 minutes since the last reset. A clean poll requires green CI, no pending review, and no unresolved actionable feedback. The first observation starts the clock; it does not count as elapsed time. Pending or unknown CI prevents completion and breaks consecutive clean polls.
