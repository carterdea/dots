...
- Review code -> diff with margin annotations and color-coded severity, plus a small module/flow diagram for unfamiliar logic.
- Plan or spec work -> one reference doc with mockups, a data-flow diagram, and annotated code snippets.
- Edit, triage, or tune -> a focused interactive tool (see Interaction and export).
...
- Reach for native HTML first: drag-to-reorder lists, `range` sliders for parameters with live preview, `form` inputs with validation, `<details>` for disclosure. Add JavaScript only where a native element cannot do the job.
- One job per tool. A triage board reorders cards; a config editor edits a config. Do not bolt on a dashboard or extra surfaces.
- **Always pair editing or tuning with an export.** Give the reader a copy-to-clipboard button that emits the result in a feedable format — JSON, Markdown, a diff, or the tuned parameter values — so the change flows back into a prompt, file, or PR and the human stays in the loop. An interactive artifact with no way to get the result out is a dead end.
- A tool is not an excuse for template tells: the anti-AI-aesthetic, color, spacing, and type rules below hold for interactive artifacts too.
