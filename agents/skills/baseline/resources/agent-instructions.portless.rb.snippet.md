...
```bash
# Rails
portless run bundle exec rails server -p $PORT -b 127.0.0.1

# Sinatra / Rack
portless run bundle exec rackup --port $PORT --host 127.0.0.1
```

Portless injects `PORT=<random 4000–4999>` and `HOST=127.0.0.1` into the child process. Pass `$PORT` explicitly on the command line — Ruby servers don't read it from the environment by default.
